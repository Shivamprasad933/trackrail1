import { ChevronRight, Clock, Gauge, ShieldCheck } from 'lucide-react';
import type { StationETA } from '@/types';
import { formatTime } from '@/lib/format';
import { PredictionRange, ConfidenceMeter } from './PredictionRange';

export function NextStationCard({ eta }: { eta: StationETA }) {
  const delay = eta.predictedDelayMinutes;
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-bold uppercase tracking-widest text-neutral-400">Next station</span>
        <span className="chip bg-accent-50 text-accent-700">
          <ChevronRight className="h-3 w-3" /> Approaching
        </span>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <div className="rounded-lg bg-neutral-50 px-3 py-2">
          <div className="flex items-center gap-1 text-[11px] font-medium uppercase text-neutral-400">
            <Clock className="h-3 w-3" /> Predicted
          </div>
